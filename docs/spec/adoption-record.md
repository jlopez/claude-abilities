...
| `source` | yes | string | The abilities repository this ability was adopted from, as `owner/repo` (or a full git URL for non-GitHub hosts). Always the repository's canonical **remote identity, never a machine-local path** — even when adoption read from a local clone, the identity comes from the clone's remote (since config schema v2 the registered entry's `source` already is this identity; see [plugin-structure.md](plugin-structure.md) and [repository-resolution.md](repository-resolution.md)). The record travels with the repo; a path is meaningless elsewhere. A remote-less experimental repository may be recorded by path only with the user's eyes-open consent and the caveat noted in the prose notes (§5). Disambiguates once multiple repositories exist; `diff`/`update`/`publish` resolve this value against the registered repositories by canonical identity match (resolution rules in [repository-resolution.md](repository-resolution.md)). |
